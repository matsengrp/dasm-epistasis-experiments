...
def load_data():
    ...
    pcp_df = train_val_df_of_multiname("v1tangCC+v1vanwinkleheavyTrainCC+v1jaffePairedCC+v1vanwinklelightTrainCC1m")

    #model_dir = os.path.expanduser("~/re/dnsm-experiments-1/dasm-train/trained_models_no_multihit")
    model_dir = os.path.expanduser("~/re/dnsm-experiments-1/dasm-train/trained_models")
    ...
    # Filter out rows with empty heavy chain sequences after branch lengths are assigned
    initial_train_len = len(train_df)
    train_df = train_df[
        (train_df["parent_heavy"].str.len() > 0) &
        (train_df["child_heavy"].str.len() > 0)
    ].reset_index(drop=True)
    if len(train_df) < initial_train_len:
        print(f"Filtered out {initial_train_len - len(train_df)} training rows with empty heavy sequences ({100*(initial_train_len - len(train_df))/initial_train_len:.2f}%)")

    initial_val_len = len(val_df)
    val_df = val_df[
        (val_df["parent_heavy"].str.len() > 0) &
        (val_df["child_heavy"].str.len() > 0)
    ].reset_index(drop=True)
    if len(val_df) < initial_val_len:
        print(f"Filtered out {initial_val_len - len(val_df)} validation rows with empty heavy sequences ({100*(initial_val_len - len(val_df))/initial_val_len:.2f}%)")

    # Compute mutation frequency on filtered dataframes
    train_df["mut_freq"] = [
        mutation_frequency(p, c)
        for p, c in zip(train_df["parent_heavy"], train_df["child_heavy"])
    ]
    val_df["mut_freq"] = [
        mutation_frequency(p, c)
        for p, c in zip(val_df["parent_heavy"], val_df["child_heavy"])
    ]
...
def main():
    ...
    plt.tight_layout()
    output_path = os.path.expanduser("~/re/dnsm-experiments-1/branch_length_regression.png")
