...
class OEPlotter(ABC):
    def __init__(
        ...
        if "imgt" in anarci_path:
            ...
            # Chothia numbering support added
            # CDRH1: [26, 32]
            # CDRH2: [52A, 55]
            # CDRH3: [96, 101]
        ...
        self.numbering, _ = get_numbering_dict(
            anarci_path, self.pcp_df, verbose=True, checks=checks
        )
    ...
    def write_sites_oe(
        ...
        # Get the appropriate ANARCI dictionary for the numbering scheme
        anarci_dict_for_scheme = get_anarci_dict(numbering_scheme)

        # Check if dataset has the requested numbering scheme
        if dataset_name not in anarci_dict_for_scheme:
            available_schemes = [
                scheme
                for scheme, scheme_dict in anarci_dicts.items()
                if dataset_name in scheme_dict
            ]
            raise ValueError(
                f"Dataset '{dataset_name}' does not have {numbering_scheme} numbering. "
                f"Available numbering schemes for this dataset: {available_schemes}"
            )

        anarci_paths = anarci_dict_for_scheme[dataset_name]
    ...
    def csp_oe_plot(self, count_ax, sub_acc):
        ...
        if sub_acc is not None:
            # Use CDR boundaries based on numbering scheme
            if self.numbering_type == "IMGT":
                cdr_bounds_list = [("27", "38"), ("56", "65"), ("105", "117")]
                xlabel = "IMGT numbering"
            elif self.numbering_type == "Chothia":
                ...
                if self.chain_type == "heavy":
                    cdr_bounds_list = [("26", "32"), ("52", "56"), ("95", "102")]
                    xlabel = "Chothia numbering (heavy)"
                elif self.chain_type == "light":
                    cdr_bounds_list = [("24", "34"), ("50", "56"), ("89", "97")]
                    xlabel = "Chothia numbering (light)"
                else:
                    # Fallback if chain type is unknown
                    cdr_bounds_list = []
                    xlabel = f"Chothia numbering ({self.chain_type})"
            else:
                # Fallback: don't shade CDR regions if numbering type unknown
                cdr_bounds_list = []
                xlabel = f"{self.numbering_type} numbering"

            for cdr_bounds in cdr_bounds_list:
                # Only shade if both boundaries exist in the numbering
                if (
                    cdr_bounds[0] in self.numbering[("reference", 0)]
                    and cdr_bounds[1] in self.numbering[("reference", 0)]
                ):
            ...
            sub_acc.set_xlabel(xlabel, fontsize=20, labelpad=10)
