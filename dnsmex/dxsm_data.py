...
def _add_suffix_variants(base_dict, suffixes):
    """Add dataset name variants with suffixes (e.g., 'dataset' -> 'dataset50k', 'dataset100k', ...)."""
    return {k + suffix: v for suffix in suffixes for k, v in base_dict.items()}


def _normalize_chain_dict(anarci_dict):
    """Convert string values to {'heavy': val} format for unlabeled chains."""
    return {
...
holdout_dict.update(_add_suffix_variants(holdout_dict, suffixes))
...
anarci_dict_imgt = _normalize_chain_dict(anarci_dict_imgt)

# Chothia numbering dictionary - populate as Chothia ANARCI files become available
anarci_dict_chothia = {
    "v1rodriguez": "DATA_DIR/v3/anarci/rodriguez-airr-seq-race-prod-NoWinCheck_igh_anarci-seqs_chothia_H.csv",
    "v1tang": "DATA_DIR/v3/anarci/tang-deepshm-prod-NoWinCheck_igh_chothia.csv",
    "v1jaffe": "DATA_DIR/v3/anarci/wyatt-10x-1p5m_fs-all-NoWinCheck_igh_chothia.csv",
}

anarci_dict_chothia.update(_add_suffix_variants(anarci_dict_chothia, suffixes))

# Assume unlabeled values are for heavy chains.
anarci_dict_chothia = _normalize_chain_dict(anarci_dict_chothia)
...
anarci_dict_imgt = _nested_dict_value_map(localify, anarci_dict_imgt)
anarci_dict_chothia = _nested_dict_value_map(localify, anarci_dict_chothia)

# Master dictionary for all numbering schemes
anarci_dicts = {
    "imgt": anarci_dict_imgt,
    "chothia": anarci_dict_chothia,
}


def get_anarci_dict(numbering_scheme="imgt"):
    """Get ANARCI dictionary for the specified numbering scheme.

    Args:
        numbering_scheme: Either "imgt" or "chothia". Default is "imgt".

    Returns:
        Dictionary mapping dataset names to ANARCI file paths for the specified scheme.
        For paired datasets, the value is a dict with "heavy" and/or "light" keys.

    Raises:
        KeyError: If the numbering scheme is not supported.

    Example:
        >>> anarci_dict = get_anarci_dict("imgt")
        >>> anarci_dict["v1tang"]["heavy"]
        'path/to/tang_imgt.csv'
    """
    if numbering_scheme not in anarci_dicts:
        raise KeyError(
            f"Numbering scheme '{numbering_scheme}' not supported. "
            f"Available schemes: {list(anarci_dicts.keys())}"
        )
    return anarci_dicts[numbering_scheme]


# Backward compatibility: maintain anarci_dict as alias to IMGT
anarci_dict = anarci_dict_imgt
