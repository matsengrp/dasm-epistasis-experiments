...
def plot_entrenched_rsa_comparison(
    ...
    for i, gene_type in enumerate(v_families):
        filtered_bg = background_data[background_data.v_family_heavy == gene_type].copy()
        filtered_hl = highlighted_data[highlighted_data.v_family_heavy == gene_type].copy()

        # Apply global categories for consistent x-axis
        filtered_bg['site'] = pd.Categorical(filtered_bg['site'], categories=global_sorted_sites, ordered=True)
        filtered_hl['site'] = pd.Categorical(filtered_hl['site'], categories=global_sorted_sites, ordered=True)
...
def plot_single_vfamily_rsa_comparison(
    ...
    filtered_bg['site'] = pd.Categorical(filtered_bg['site'], categories=sorted_sites, ordered=True)
    filtered_hl['site'] = pd.Categorical(filtered_hl['site'], categories=sorted_sites, ordered=True)
...
def plot_rsa_summary_2x2(
    ...
    for i, gene_type in enumerate(v_families):
        ...
        filtered_bg['site'] = pd.Categorical(filtered_bg['site'], categories=vfamily_sorted_sites, ordered=True)
        filtered_hl['site'] = pd.Categorical(filtered_hl['site'], categories=vfamily_sorted_sites, ordered=True)
...
def examine_site(
    ...
    """
    cur_df = df[(df.is_germline == True) & (df.site == site) & (df.v_family_heavy == vfamily)]
    ...
    sns.countplot(cur_df, x='amino_acid', hue='amino_acid', palette=palette_aa, ax=ax[0])
    ...
    sns.boxplot(cur_df, x='amino_acid', y='rsa_heavy_light_antigen', showfliers=False,
                hue='amino_acid', palette=palette_aa, ax=ax[3], whis=[5, 95])
    ...
    if comparison_type == 'within':
        ...
        for x, y, label in points:
            ...
            if has_point_to_right:
                ax[1].text(x - x_offset, y, label, fontsize=ANNOT_SIZE, ha='right', va='center', zorder=5)
            else:
                ax[1].text(x + x_offset, y, label, fontsize=ANNOT_SIZE, ha='left', va='center', zorder=5)
...
def plot_ramachandran_by_site_range(
    ...
    sites_to_plot = [str(i) for i in range(center_site_int - site_range // 2, center_site_int + site_range // 2 + 1)]
        ...
        for col_idx, site in enumerate(sites_to_plot):
            ...
            if len(data) > 0:
                if palette_aa is not None:
                    ...
                    sns.scatterplot(
                        data=data, x='phi', y='psi', hue='amino_acid',
                        ax=ax, alpha=0.4, s=50, edgecolor='black', linewidth=0.5,
                        legend=False
                    )

                ax.axhline(y=0, color='lightgray', linestyle='--', linewidth=0.5, alpha=0.3)
                ax.axvline(x=0, color='lightgray', linestyle='--', linewidth=0.5, alpha=0.3)
                ax.set_xlim(-180, 180)
                ax.set_ylim(-180, 180)
                ax.set_aspect('equal')
    ...
    if save_fig:
        import os
        os.makedirs(output_dir, exist_ok=True)
        fig.savefig(f'{output_dir}/site_{center_site}_angle_analysis.png', bbox_inches='tight', dpi=300)
