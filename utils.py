...
def add_germline_information(pcp_df, site_df, germline_codons_path="germline/germline_codons.csv"):
    """
    ...
    pcp_df_for_merge = pcp_df_work[['v_gene', 'j_gene', 'v_family',]].copy()
    pcp_df_for_merge['pcp_index'] = pcp_df_for_merge.index
    
    # Merge family information
    site_df_enhanced = pd.merge(site_df_enhanced, pcp_df_for_merge, on='pcp_index', how='inner')
    ...
    germline_for_merge = germline_codons_df.rename(columns={
        'codon': 'germline_codon', 
        'amino_acid': 'germline_amino_acid'
    })
