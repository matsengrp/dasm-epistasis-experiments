...
1. Deep mutational scanning (DMS) experiments that measure binding or expression for thousands of variants. Limitation: are labor-intensive and so limited to specific antibodies and antigens. DMS also allows testing only for point mutations, and this method allows us to test multiple mutations and consecutive mutations at the same time.
2. Computational predictions based on structural models are possible, but these might not be accurate enough to catch the complex interactions within the protein in its bound and unbound states. Looking at structure alone does not usually catch the tradeoffs between stability and function.
...
- Johanne Jacobson lab
- Perhaps collaborate with David Glass from Newell lab on HIV antibodies
...
* Interesting point - our current DASM model is trained mostly on pre covid data. How well would it do on predicting epistasis and fitness in general on the Shculz dataset? How better would a model that was trained on covid data do?

## Are there any other categorically different approaches that could be applied here?
If one had a structure one could use https://www.science.org/doi/10.1126/science.add2187 or https://www.biorxiv.org/content/10.1101/2024.07.09.602403v2 (Hermes). It would be interesting to compare to these, both with a "real" PDB structure and also with an AlphaFold structure.


## If this is a methods project, what methods will you compare to? Can you get them running before writing new code?
Compare to stability predictions according to structure.
